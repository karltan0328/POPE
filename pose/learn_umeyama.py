import json
import os
from pose.dataset import NocsDatasetm, pose_dataset
from torch.utils.data import DataLoader
from pose.pose_utils import collate_fn, data_prefetcher, geodesic_distance, o6d2mat, qua2mat
import torch
...
class Model(nn.Module):
    def __init__(self, mode: str='matrix'):
        ...
        self.N = 3
        self.L = 9
        self.embedding = Embedding(in_channels=self.N, N_freqs=self.L)
        self.transformerlayer = nn.TransformerEncoderLayer(d_model=self.N * (self.L * 2 + 1), nhead=1)
        ...
        self.linear2 = nn.Linear(in_features=128, out_features=16)
        ...
        self.mlp = nn.Sequential(
            nn.Linear(in_features=self.N * (self.L * 2 + 1), out_features=self.inner_size),
            nn.LeakyReLU(),
...
with open('data/pairs/LINEMOD-test.json') as f:
    LM_dataset_points_pair_dir_list = json.load(f)
with open('data/pairs/Onepose-test.json') as f:
    onepose_points_pair_dir_list = json.load(f)
with open('data/pairs/OneposePlusPlus-test.json') as f:
    oneposeplusplus_points_pair_dir_list = json.load(f)

paths = [LM_dataset_points_path, onepose_points_path, oneposeplusplus_points_path]
...
torch.save(net, f'pose_weight/test.pt')
